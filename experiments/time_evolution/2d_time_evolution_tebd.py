...
def time_evolve(p0, N, dt, U, ops=[X, Y, Z], get_energy=True, mode="TEBD"):
    ...
    energies = []
    evs = []
    objx = obj_TEBDx if mode == "TEBD" else obj_localx
    objy = obj_TEBDy if mode == "TEBD" else obj_localy
    for _ in range(N):
        if get_energy:
...
plt.tight_layout()
plt.savefig("../figs/2d_time_evolution_tebd.pdf")
np.save('../data/2d_energies_tebd', energies)
np.save('../data/2d_evs_tebd', evs)
